...
public class LoadMeteorites {
	public static void main(String[] args) throws IOException{
		...
		File file = new File("refdata/meteorites.xls");
		FileInputStream inputStream = new FileInputStream(file);
		
		try {
			HSSFWorkbook workbook = new HSSFWorkbook(inputStream);
			HSSFSheet sheet = workbook.getSheet("METEORITES");
			...
			while (rowIterator.hasNext()) {
				...
				if(row.getRowNum()>= beginRowNum) {
					...
					String sfName = XlsParser.formatString(XlsParser.getCellValueString(row.getCell(1)));
					...
					*/
					...
					String materialCode = XlsParser.getCellValueString(row.getCell(2));
					...
					String tcName = XlsParser.getCellValueString(row.getCell(3));
					if (tcName != null) {
